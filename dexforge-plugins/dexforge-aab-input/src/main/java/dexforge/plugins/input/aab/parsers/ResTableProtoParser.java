...
public class ResTableProtoParser implements IResTableParser {
	private final RootNode root;
	private final AaptProtoParser protoParser = new AaptProtoParser();
		...
		for (Type type : types) {
			...
			for (Entry entry : type.getEntryList()) {
				...
				for (ConfigValue configValue : entry.getConfigValueList()) {
					String config = protoParser.parse(configValue.getConfig());
					...
					if (configValue.getValue().getValueCase() == Value.ValueCase.ITEM) {
						protoValue = new ProtoValue(protoParser.parse(configValue.getValue().getItem()));
					} else {
						protoValue = protoParser.parse(configValue.getValue().getCompoundValue());
					}
