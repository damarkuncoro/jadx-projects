...
import jadx.gui.device.adb.ADB.JDWPProcessListener;
import jadx.gui.device.adb.ADB.Process;

import static jadx.gui.device.adb.ADB.ADB_CHARSET;
...
public class ADBDevice {
	...
	public List<String> getProp(String entry) throws IOException {
		...
		try (Socket socket = ADB.connect(info.getAdbHost(), info.getAdbPort())) {
			...
			if (payload != null) {
				props = jadx.gui.device.adb.parsers.AndroidPropsParser.parse(new String(payload, ADB_CHARSET));
			}
