...
public class MockDeviceExplorerService implements DeviceExplorerService {
	...
	public MockDeviceExplorerService() {
		devices.add(new DeviceInfo("mock-device-1 device product:Mock_Pixel_6 model:Mock_Pixel_6 device:mock-device-1 transport_id:1", "localhost", 5037));
		devices.add(new DeviceInfo("mock-device-2 offline product:Mock_Pixel_5 model:Mock_Pixel_5 device:mock-device-2 transport_id:2", "localhost", 5037));
		devices.add(new DeviceInfo("mock-device-3 unauthorized product:Mock_Pixel_4 model:Mock_Pixel_4 device:mock-device-3 transport_id:3", "localhost", 5037));
	}
		...
		for (DeviceInfo dev : devices) {
			if (dev.getSerial().equals(serial)) {
				if ("offline".equals(dev.getState())) {
					throw new DeviceExplorerException(DeviceExplorerException.DeviceExplorerErrorCode.DEVICE_OFFLINE, "Device is offline: " + serial);
				}
				if ("unauthorized".equals(dev.getState())) {
					throw new DeviceExplorerException(DeviceExplorerException.DeviceExplorerErrorCode.UNAUTHORIZED, "Device is unauthorized: " + serial);
				}
				return;
			}
		}
	...
	public List<ApkPath> resolveApkPaths(String serial, String packageName, int userId) throws IOException, DeviceExplorerException {
		...
		throw new DeviceExplorerException(DeviceExplorerException.DeviceExplorerErrorCode.PACKAGE_NOT_FOUND, "Package not found: " + packageName);
	}
	...
	@Override
	public PullResult pullAndDecompile(String serial, String packageName, String outDir, int userId) throws IOException, DeviceExplorerException {
