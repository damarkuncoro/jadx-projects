...
public class FridaDownloader {
	...
	public void downloadFridaGadget(String arch, String version) throws IOException {
		try {
			...
			String downloadUrl = "https://github.com/frida/frida/releases/download/" + version + "/frida-gadget-" + version + "-android-"
					+ arch + ".so.xz";
	...
	public String downloadAndPushFridaServer(jadx.gui.device.adb.ADBDevice device) {
		try {
			...
			if (!localBinary.exists()) {
				...
				String downloadUrl = "https://github.com/frida/frida/releases/download/" + version + "/frida-server-" + version
						+ "-android-" + arch + ".xz";
