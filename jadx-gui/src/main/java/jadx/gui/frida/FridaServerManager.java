	...
	public boolean autoStartFridaServer() {
		try {
			...
			for (ADBDevice device : devices) {
				...
				if (!isRooted) {
					logAppender.accept("[WARN] Device " + serial
							+ " is not rooted. Automated frida-server startup is not supported on jailed devices.");
				...
				try {
					...
					if (exitCode != 0) {
						logAppender.accept(
								"[WARN] Could not start frida-server automatically on " + serial + ". You may need to start it manually.");
					} else {
