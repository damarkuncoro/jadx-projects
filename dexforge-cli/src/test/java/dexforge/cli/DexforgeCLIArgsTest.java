...
public class DexforgeCLIArgsTest {

	private static final Logger LOG = LoggerFactory.getLogger(DexforgeCLIArgsTest.class);
	...
	public void testOptionsOverride() {
		assertThat(override(new DexforgeCLIArgs(), "--no-imports").isUseImports()).isFalse();
		assertThat(override(new DexforgeCLIArgs(), "--no-debug-info").isDebugInfo()).isFalse();
		assertThat(override(new DexforgeCLIArgs(), "").isUseImports()).isTrue();

		DexforgeCLIArgs args = new DexforgeCLIArgs();
	...
	private DexforgeCLIArgs parse(String... args) {
		return parse(new DexforgeCLIArgs(), args);
	}

	private DexforgeCLIArgs parse(DexforgeCLIArgs jadxArgs, String... args) {
		return check(jadxArgs, jadxArgs.processArgs(args));
	}

	private DexforgeCLIArgs override(DexforgeCLIArgs jadxArgs, String... args) {
		return check(jadxArgs, overrideProvided(jadxArgs, args));
	}

	private static boolean overrideProvided(DexforgeCLIArgs jadxArgs, String[] args) {
		JCommanderWrapper jcw = new JCommanderWrapper(new DexforgeCLIArgs());
		if (!jcw.parse(args)) {
			return false;
		}
	...
	private static DexforgeCLIArgs check(DexforgeCLIArgs jadxArgs, boolean res) {
