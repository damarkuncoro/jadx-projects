...
public class DexforgeConfigAdapter<T extends IDexforgeConfig> {
	...
	public DexforgeConfigAdapter(Class<T> configCls, String defaultConfigName) {
		this(configCls, defaultConfigName, gsonBuilder -> {
		});
	}

	public DexforgeConfigAdapter(Class<T> configCls, String defaultConfigName, Consumer<GsonBuilder> applyGsonOptions) {
	...
	private Path resolveConfigRef(String configRef) {
		if (configRef == null || configRef.isEmpty()) {
			...
			return DexforgeCommonFiles.getConfigDir().resolve(defaultConfigFileName);
		}
		...
		return DexforgeCommonFiles.getConfigDir().resolve(configRef + ".json");
	}
}
