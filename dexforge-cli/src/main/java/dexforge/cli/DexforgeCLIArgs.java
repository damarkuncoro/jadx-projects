...
public class DexforgeCLIArgs implements IDexforgeConfig {
	private static final Logger LOG = LoggerFactory.getLogger(DexforgeCLIArgs.class);
	 ...
	 * prefer {@link #processArgs(String[], DexforgeCLIArgs, DexforgeConfigAdapter)}
	 */
	public boolean processArgs(String[] args) {
		return processArgs(args, this, null) != null;
	}

	public static <T extends DexforgeCLIArgs> @Nullable T processArgs(
			String[] args, T argsObj, @Nullable DexforgeConfigAdapter<T> configAdapter) {
	...
	private static <T extends DexforgeCLIArgs> void applyArgs(T argsObj) {
		// apply log levels
	...
	private static void printFilesAndDirs(String defaultConfigFileName) {
		...
		System.out.println(" - default config file: " + DexforgeCommonFiles.getConfigDir().resolve(defaultConfigFileName).toAbsolutePath());
		System.out.println(" - config directory:    " + DexforgeCommonFiles.getConfigDir().toAbsolutePath());
		System.out.println(" - cache directory:     " + DexforgeCommonFiles.getCacheDir().toAbsolutePath());
		System.out.println(" - temp directory:      " + DexforgeTempFiles.getTempRootDir().getParent().toAbsolutePath());
	}
	...
	private static <T extends DexforgeCLIArgs> void saveConfig(T argsObj, @Nullable DexforgeConfigAdapter<T> configAdapter) {
		if (configAdapter == null) {
